...
(function () {
  let openTerminalModalFn = null;
  let closeTerminalModalFn = null;

  window.openTerminalModal = function () {
    if (typeof openTerminalModalFn === 'function') {
      openTerminalModalFn();
    } else {
      setTimeout(() => {
        if (typeof openTerminalModalFn === 'function') openTerminalModalFn();
      }, 100);
    }
  };

  window.closeTerminalModal = function () {
    if (typeof closeTerminalModalFn === 'function') {
  ...
  function initTerminal() {
    ...
    if (termModalBackdrop) {
      termModalBackdrop.addEventListener('click', closeTerminalModalFn);
    }
...
<div class="term-line">📞 <span class="term-accent">Phone:</span> +91 9422446157</div>`
      ...
      exit: {
        ...
        exec: () => {
          setTimeout(closeTerminalModalFn, 200);
